...
public class HibernateDelete {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//session.delete(student3.getId());
			//System.out.println("Retrived from DB -"+session.get(Student.class, student1.getId()));
			session.createQuery("delete from Student where id="+student2.getId());
			session.getTransaction().commit();
		}catch(Exception e) {
